       ...
       function DEsWeights = getDEsWeights(this)
        ...
        function fitness = train(this, model_name, max_run_time,save_figures)
            ...
            %% construct prosthesis
             prost = prosthesis(0,135); %@ m ok wichtig!!!! nicht 180°
		...
		time=1000;
            ...
            %% return result
            % todo hier sollte die benoetigte zeit als fitness
            % zurueckgegeben werden, wenn not reached dann sehr hohe zeit
            %successfully = prost.reached_angles(1) && prost.reached_angles(2);
            successfully=true;
		if(successfully)
                % @M todo  besser fitness an anzahl der erreichten winkel
                % messen.
                fitness = 1-time/max_run_time; % wert zwischen 0 und eins. 0 schlecht 1 gut
            else
                fitness =0.000001;
            end
            %fprintf('not delete prost')
           % clear('prost')% sollte eig nichts aendern da bei verlassen der methode eh geloescht werden sollte
            this.reset();
        end
