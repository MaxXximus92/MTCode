   ...
   function this = spikenet(spikingThreshold,settingsFile,savePath,equationParams, weightsMatrix)
        ...
        function fitness = simulate(this, model_name, angle_start, angles_to_simulate, time_per_angle)
           ...
           % num_angles2 = length(prost.angles_to_simulate)*prost.angle_simulation_time/50;
            ...
            savefig([this.savePath sprintf('%s (firings)NewFitness=%0.5f.fig',model_name,fitness)]);
            set(0,'CurrentFigure',prost.fig);
            xlim([0,time]);
            box on;
            saveas(gcf,[this.savePath sprintf('%s (movement)NewFitness=%0.5f.pdf',model_name,fitness)], 'pdf');
            savefig([this.savePath sprintf('%s (movement)NewFitness=%0.5f.fig',model_name,fitness)]);

            %% return result
            % fitness = percentage of reached angles  man könnte die zeit
            % noch als 2 ten faktor einbauen
            
            %% calculate box plot comparison data
            % time to reach each angle 
            % rmsd of each angle from 20 to 30 seconds
            % total rmsd
            ...
            angleRMSDs = [];
            anglesReachTimes = [];
            for i= 1:length(prost.angles_to_simulate)
                angle_target= prost.angles_to_simulate(i);
                maxt=angle_start_time+prost.time_per_angle;
                angles=ahistory(ahistory(:,1)>=angle_start_time+prost.time_per_angle*1/3& ahistory(:,1)<maxt,2);
                
                
                currentSquaredSum= (sum((angles-angle_target).^2));
                currentRmsd = sqrt(currentSquaredSum/length(angles));
                angleRMSDs = [angleRMSDs currentRmsd];
                
                timedif =-1;
                if prost.time_reached_angles(i) ~=0
                timedif= prost.time_reached_angles(i)-angle_start_time;
                end
                anglesReachTimes = [anglesReachTimes timedif];
                
                squared_sum= squared_sum+currentSquaredSum;
                num_angles = num_angles + length(angles);
                
                angle_start_time =maxt;
            end
            totalRmsd= sqrt(squared_sum/num_angles);
            compareData =struct('anglesToSimulate',prost.angles_to_simulate,'angleRMSDs',angleRMSDs,'anglesReachTimes',anglesReachTimes,'totalRmsd',totalRmsd,'totalRMSDfullTime',rmsd);
            save([this.savePath sprintf('%s_compareData.mat',model_name)], '-struct', 'compareData');
