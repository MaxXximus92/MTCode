       ...
       protected ulong evaluationCount = 0;
        ...
        public static void evalNet(Object input)
        {
            ...
            {	// Future genomes may not decode - handle the possibility.
                e.g.addFitness(EvolutionAlgorithm.MIN_GENOME_FITNESS);
            }
            else
            {
                ...
                //sem2.Release();
                int numNeurons;
                e.g.addFitness(Math.Max(e.NetworkEvaluator.threadSafeEvaluateNetwork(network,sem2,out numNeurons), EvolutionAlgorithm.MIN_GENOME_FITNESS));
                sem2.WaitOne();
                Console.WriteLine("Genome Number {0} NumNeurons {1} fitness {2:0.#####} dt {3} ", e.nu,numNeurons,e.g.MeanFitness ,(DateTime.Now.Subtract(dt)));
                sem2.Release();
            }
