...
//		}
		...
		private IGenome RouletteWheelSelect(Species species)
		{
			...
			for(int genomeIdx=0; genomeIdx<genomeBound; genomeIdx++)
			{
				...
				accumulator += genome.MeanFitness;
...
//		}
		...
		private void UpdateFitnessStats()
		{
			...
			foreach(Species species in pop.SpeciesTable.Values)
			{
				...
				if(genome.MeanFitness > bestFitness)
				{
					bestGenome = genome;
					bestFitness = bestGenome.MeanFitness;
				}

				// Track the generation number when the species improves.
				if(genome.MeanFitness > species.MaxFitnessEver)
				{
					species.MaxFitnessEver = genome.MeanFitness;
				...
				//	Debug.Assert(genome.Fitness>=EvolutionAlgorithm.MIN_GENOME_FITNESS, "Genome fitness must be non-zero. Use EvolutionAlgorithm.MIN_GENOME_FITNESS");
					species.TotalFitness += genome.MeanFitness;

					if(genomeIdx < species.SelectionCount)
						species.SelectionCountTotalFitness += genome.MeanFitness;
			...
			if(bCandidateCullFlag)
			{
				...
				foreach(Species species in pop.SpeciesTable.Values)
				{
					if(species.Members[0].MeanFitness == bestFitness)
			...
			{
				Debug.WriteLine("UpdateStats() - bestFitness=" + bestGenome.MeanFitness.ToString() + ", " + bestFitness.ToString());
				pop.MaxFitnessEver = bestGenome.MeanFitness;
