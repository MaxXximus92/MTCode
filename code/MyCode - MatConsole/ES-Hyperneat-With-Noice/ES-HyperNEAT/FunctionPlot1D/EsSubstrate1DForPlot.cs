...
namespace EsExperimentNS
{
    ...
    public class EsSubstrate1DForPlot : EvolvableSubstrate1D
    {
        private int esIterations;

        public EsSubstrate1DForPlot(INetwork genome)
        {
        ...
        public double[][] scanValues(int resolution)
        {
            double[][] weights = new double[resolution][];
            for (int i = 0; i < resolution; i++)
            {
                weights[i] = new double[resolution];
            }

            float dist = 2.0f / (resolution - 1); // -1 to get a neuron at 1 and at 0
            ...
            for (int i = 0; i < resolution; i++)
            {
                ...
                for (int j = 0; j < resolution; j++)
                {
                    ...
                    double output = genome.GetOutputSignal(0) * weightRange;
        ...
        public double[] getNeuronCodingValues(int outputnum, int resolution)
        {
            double[] weights = new double[resolution];
            ...
            for (int i = 0; i < resolution; i++)
            {
                    ...
                    coordinates[2] = mod1coord;
                    genome.ClearSignals();
                    genome.SetInputSignals(coordinates);
                    genome.MultipleSteps(iterations);
                    double output = genome.GetOutputSignal(outputnum) * weightRange;
        ...
        public double[] getNeuronCodingValues(int outputnum, List<Neuron> neurons )
        {
        ...
        public double[][] GetConnections(out List<Neuron> neurons)
        {
         ...
         // first numInputNeurons found source neurons are D neurons
           ...
           // List<Neuron> neurons = neuronDictionary.Values.ToList();
            ...
            double a = HyperNEATParameters.varianceThreshold;
            ...
            List<Neuron> clearedNeurons = new List<Neuron>();
            ...
            {
                foreach (Neuron n in clearedNeurons)
                {
                    if (n.type == type)
                    {
                        n.index = counter++;
            ...
            Debug.Assert(counter == clearedNeurons.Count);

            //allNeurons.Where(n => n.toDelete == false).ToList();

            // build connection matrix  
            int matrixDim = clearedNeurons.Count;
            double[][] connectionsMat = new double[matrixDim][];
            for (int i = 0; i < matrixDim; i++)
            {
                connectionsMat[i] = new double[matrixDim];
            }
            NType[] typeArray = new NType[matrixDim];
            double[] positionArray = new double[matrixDim];
            ...
            foreach (Neuron neuron in clearedNeurons)
            {
                ...
                foreach (Neuron outgoing in neuron.outCon)
                {
                    ...
                    {//Fehler !outgoing connection index kann -1 sein, ka wo der herkommt, sollte nicht passieren. so ist es erstmal gefixt
                        ...
                        coordinates[0] = neuron.pos;
                        coordinates[1] = outgoing.pos;
                        genome.ClearSignals();
                        genome.SetInputSignals(coordinates);
                        genome.MultipleSteps(iterations);
                        double output = genome.GetOutputSignal(0) * weightRange;

                        connectionsMat[neuron.index][outgoing.index] = output;
                    }
                }
            }
