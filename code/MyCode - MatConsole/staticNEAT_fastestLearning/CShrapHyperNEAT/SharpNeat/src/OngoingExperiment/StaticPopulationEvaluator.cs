...
{
    public class StaticPopulationEvaluator : MultiThreadedPopulationEvaluator
    {

        public StaticPopulationEvaluator(INetworkEvaluator eval)
            : base(eval, null)
        {
        ...
        public void simulateAndPlot(IGenome[] g, string[] names)
        {
            for (int i = 0; i < g.Length; i++)
            {
                ...
                sem.WaitOne();
                evalPack2 e = new evalPack2(networkEvaluator, activationFn, g[i], i, "simulate_"+names[i]);
        ...
        public void trainAndMatlabPlot(IGenome[] g, string[] names)
        {
            for (int i = 0; i < g.Length; i++)
            {
                ...
                sem.WaitOne();
                evalPack2 e = new evalPack2(networkEvaluator, activationFn, g[i], i, names[i]);
        ...
        public static void simulateNet(Object input)
        {
        ...
        public static void plotNet(Object input)
        {
            ...
            sem2.Release();
            DateTime dt = DateTime.Now;
            ((StaticNetworkEvaluator)(e.NetworkEvaluator)).plotMatlabGraphsAndSafeNetwork(network, e.Name);
