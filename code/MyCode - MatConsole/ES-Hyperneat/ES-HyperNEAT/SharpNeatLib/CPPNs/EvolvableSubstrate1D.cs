...
namespace SharpNeatLib.CPPNs
{
     ...
     **/
    public class EvolvableSubstrate1D
    {
        ...
        {
            public float x, y;
            public float w; //stores the CPPN value
        ...
        public class TempConnection
        {
            ...
            public float weight;
            public TempConnection(float x1, float x2, float weight)
            {
         ...
         */
        public QuadPoint QuadTreeInitialisation( int initialDepth, int maxDepth)
        {
            QuadPoint root = new QuadPoint(0, 0, 2, 1); //x, y, width, level
            ...
            while (queue.Count > 0)
            {
                ...
                {
                    c.w = queryCPPN(c.x, c.y); // Outgoing connectivity pattern
                }
        ...
        public void PruneAndExpress( ref List<TempConnection> connections, QuadPoint node, float maxDepth)
        {
            ...
            foreach (QuadPoint c in node.childs)
            {
                ...
                {
                    PruneAndExpress(ref connections, c, maxDepth);
                }
                else //this should always happen for at least the leaf nodes because their variance is zero  
                {
                    // Determine if point is in a band by checking neighbor CPPN values
                        left = Math.Abs(c.w - queryCPPN( c.x - node.width, c.y));
                        right = Math.Abs(c.w - queryCPPN( c.x + node.width, c.y));
                        top = Math.Abs(c.w - queryCPPN( c.x, c.y - node.width));
                        bottom = Math.Abs(c.w - queryCPPN( c.x, c.y + node.width));
                


                    if (Math.Max(Math.Min(top, bottom), Math.Min(left, right)) > bandThreshold)
                    {
                        TempConnection tc;
                            tc = new TempConnection( c.x, c.y, c.w);
        ...
        public float queryCPPN(float x1, float x2)
        {
