classdef spikenet < handle
    ...
    methods
        ...
        % experimenten
        function this = spikenet(spikingThreshold,settingsFile,savePath,scaleFactors,equationParams, weightsMatrix)
            if(nargin>4)
            ...
            if (nargin > 0)
                ...
                this.loadSettingsFile(settingsFile,scaleFactors);
...
%         end
        ...
        function neuronTypes = getNeuronTypes(this)
            if (~isempty(this.neuronTypes))
                ...
                numNeurons = sum(cell2mat(values(this.neuronRatios)));
                neuronTypes = cell(1,numNeurons);
                current = 0;
        ...
        function loadSettingsFile(this, filename,scaleFactors)
            ...
            % load neuron types
            neuTypes = this.getNeuronTypes();
            % scale them.
            types = keys(this.neuronRatios);
            scaledTypes = [];
            for i = 1:length(types) 
            spesType = find(strcmp(neuTypes,types(i)));
               scaled= scaleRep(spesType,scaleFactors(i));
               scaledTypes = [scaledTypes scaled];
            end
            this.neuronTypes= neuTypes(scaledTypes);
            this.numNeurons= size(this.neuronTypes,2);
