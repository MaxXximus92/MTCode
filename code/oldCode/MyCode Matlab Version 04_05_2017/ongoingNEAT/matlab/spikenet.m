classdef spikenet < handle
    ...
    methods
        % constructor
        function this = spikenet(numNeurons,spikingThreshold,settingsFile,savePath)
            if (nargin > 0)
        ...
        function rmsds = simulate( this, model_number, run_time, run_settings, trials_per_settings,save_trail_figures ,save_mean_figures)
            if nargin < 6
            ...
            for run_number=1:size(run_settings,2)
                ...
                for trial_number=1:trials_per_settings
                    ...
                    if save_trail_figures
                        ...
                        box on;
                        set(gca,'fontsize',10);
                        saveas(gcf,[this.savePath sprintf('Model %03.0f - Run %.0f - Trial %.0f - firings (start %.0f - target %.0f).pdf',model_number,run_number,trial_number,run_settings(1,run_number),run_settings(2,run_number))], 'pdf');
                        ...
                        saveas(gcf,[this.savePath sprintf('Model %03.0f - Run %.0f - Trial %.0f - movement (start %.0f - target %.0f).pdf',model_number,run_number,trial_number,run_settings(1,run_number),run_settings(2,run_number))], 'pdf');
                    end
                ...
                if save_mean_figures
                    ...
                    saveas(gcf,[this.savePath sprintf('Model %03.0f - Run %.0f - mean movement (start %.0f - target %.0f).pdf',model_number,run_number,run_settings(1,run_number),run_settings(2,run_number))], 'pdf');
                end
                %% calculate average RMSD
%                 rmsds(run_number) = mean(sqrt(mean((prost.angle_target - movements(movements(:,1)>=20000,2:end)).^2)));
                           rmsds(run_number) = mean(sqrt(mean((prost.angle_target - movements(movements(:,1)>=run_time*0.8,2:end)).^2)));
            end
        end
...
%         end
        ...
        function [a,b,d] = getEquationParameters(this)
             %if there is no defined matrix, create one.
                if exist([this.savePath 'EquationParams.mat'],'file')==2
                    eqParamsStruct = load([this.savePath 'EquationParams.mat'],'a','b','d');
                    a=eqParamsStruct.a;
                    b=eqParamsStruct.b;
                    d=eqParamsStruct.d;
                    ...
                    save([this.savePath 'EquationParams.mat'],'a','b','d');
                end
        end
        ...
        function weightsMatrix = getWeightMatrix(this)
            if (~isempty(this.weightsMatrix))
                ...
                %if there is no defined matrix, create one.
                if exist([this.savePath 'weightsMatrix.mat'],'file')==2
                    weightsMatrixStruct = load([this.savePath 'weightsMatrix.mat']);
        ...
        function weightsMatrix = generateWeightMatrix(this)
                ...
                save([this.savePath 'weightsMatrix.mat'],'weightsMatrix');
