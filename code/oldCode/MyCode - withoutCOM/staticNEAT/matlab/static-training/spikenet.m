classdef spikenet < handle
    ...
    methods
        ...
        function numdEs = getDEsNum(this)
                  numdEs(1)= length(this.getCellsOfType('D'));
                  numdEs(2)= length(this.getCellsOfType('EM'));
        end
        ...
        function setDEsWeights(this, weights)
            ...
            if dl== length(dCells) && esl== length(esCells)
       ...
       function DEsWeights = getDEsWeights(this)
        ...
        function fitness = train(this, model_name, max_run_time,save_figures)
            ...
            xlim([0,time]);
            box on;
            saveas(gcf,sprintf('Model %s (firings).pdf',model_name), 'pdf');
            set(0,'CurrentFigure',prost.fig);
            xlim([0,time]);
            box on;
            saveas(gcf,sprintf('Model %s (movement).pdf',model_name), 'pdf');
            close all hidden
            end
            %% return result
            % todo hier sollte die benötigte zeit als fitness
            % zurückgegeben werden, wenn not reached dann sehr hohe zeit
            successfully = prost.reached_angles(1) && prost.reached_angles(2);
            if(successfully)
               fitness = 1-time/max_run_time; % wert zwischen 0 und eins. 0 schlecht 1 gut
            else
               fitness =0; 
            end
        end
